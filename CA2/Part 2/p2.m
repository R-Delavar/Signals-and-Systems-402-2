...
%%
%part4: removing small objects + background
figure
picture = bwareaopen(picture,500);
subplot(1,3,1)
imshow(picture)
background=bwareaopen(picture,4100);
subplot(1,3,2)
imshow(background)
...
hold off
%% 
% part6: Loading the mapset
load PTRAININGSET;
totalLetters=size(PTRAIN,2);

%%
% part7: correlation of each part with the mapset
final_output=[];
t=[];
for n=1:Ne
    [r,c]=find(L==n);
